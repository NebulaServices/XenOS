  ...
  register(appName, posX, posY, location, native) {
    ...
    if (check === null) {
      if ((appName, posX, posY == null)) {
        ...
        const os_desk = document.getElementById("os-desktop");
        try {
          let injectCode = `const thisAppName = this.dataset.appname;xen.windowManager.focus(thisAppName);/*xen.windowManager.modifyWindow(thisAppName, "zIndex", this.style.zIndex);*/xen.windowManager.modifyWindow(thisAppName, "location_x", this.style.left);xen.windowManager.modifyWindow(thisAppName, "location_y", this.style.top);`
   ...
   xen.system.unregister("${appName}")
    document.dispatchEvent(
          ...
          let miniCode = `xen.apps.minimize('${appName}');`;
