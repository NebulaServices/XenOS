...
window.__XEN_WEBPACK.core.DockComponent = class DockComponent {
  ...
  async opened(app) {
    ...
    if (this.pins.includes(app)) {
      ...
      document
        .getElementById("_Dock_" + meta.name)
        .querySelector("img").onclick = function () {};
    } else {
      await this.#add(app);
  ...
  async icon(app, url) {
    ...
    if (document.getElementById("_Dock_" + meta.name)) {
      document.getElementById("_Dock_" + meta.name).querySelector("img").src =
        url;
    }
  }
    ...
    function cb(event) {
      try {
        if (
          ...
          !document.querySelector(".start-menu").contains(event.target)
        ) {
          if (document.getElementById("startButton").contains(event.target))
            return;
    ...
    if (e.type == "keyup")
      if (e.key == "Enter") {
        if (that.selectedItem) {
          ...
          xen.apps.launch(
            document.querySelector(".start-results .start-app").dataset.app
          );
  ...
  async generateSearch(term) {
    ...
    const apps = await Promise.all(
      (
        await (await fetch("/apps/data")).json()
      ).map(
        async (e) => await (await fetch("/apps/" + e + "/manifest.json")).json()
      )
    );
    var names = apps.map((e) => e.name);

    if (names.find((e) => e.toLowerCase().includes(term.toLowerCase()))) {
      data.apps = names.filter((e) =>
        e.toLowerCase().includes(term.toLowerCase())
      );
    }

    var html = `<div class="search-apps">${data.apps
      .map((e) => apps.find((g) => g.name == e))
      .map(
        (e) =>
          `<div class="start-app" data-app="${
            e.id
