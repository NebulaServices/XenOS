...
appWin.getAllWindows = () =>
	Object.entries(xen.windowManager.windows).map(([_name, window]) =>
		window.native ? new _NativeWindow(window) : window
	);
...
window.__XEN_WEBPACK.core.AppLoaderComponent = class ALC {
	...
	constructor() {
		document.addEventListener("WindowClose", close => {
