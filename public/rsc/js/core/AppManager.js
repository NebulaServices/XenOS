...
window.__XEN_WEBPACK.core.AppManagerComponent = class AMC {
  ...
  async start() {
    ...
    if (!(await xen.fs.exists("__APPS.xen")))
      await xen.fs.writeFile("__APPS.xen", "[]");
  ...
  async #nativeInstall(pkg, repo, log) {
      ...
      for (let asset of meta.assets) {
        promises.push(
          new Promise(async (resolve) => {
            ...
            await that.#install(author, project, asset, body, meta.entry, log);
  ...
  createDrag(name) {
    var that = this;
    this.dragData[name] = {
      startX: this.minDown[name].clientX - this.minDown[name].target.offsetLeft,
      startY: this.minDown[name].clientY - this.minDown[name].target.offsetTop,
    };

    this.minDown[name].target
      .querySelectorAll("*")
  ...
  minClick(event, name) {
    if (
      event.clientX == this.minDown[name].clientX &&
      event.clientY == this.minDown[name].clientY
    ) {
