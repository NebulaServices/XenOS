...
function createObject(db) {
  return new Promise(async (resolve) => {
    var req = db.createObjectStore('main', { keyPath: 'id', autoIncrement: true });
    return resolve(req);
  });
}
...
function saveFile(db, file, content) {
  return new Promise(async (resolve) => {
    return resolve(db.transaction('main', 'readwrite').objectStore('main').add({key: file, body: content}));
  });
}
