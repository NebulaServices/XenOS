    ...
    (async () => {
        ...
        return new Response(
          JSON.stringify(
            (await (await caches.open("apps")).keys())
              .filter((e) => e.url.endsWith("/manifest.json"))
              .filter((e) => e.url.split("/").length > 4)
              .map((e) => e.url.split("/").slice(4, 6))
              .map((e) => e.join("/"))
          ),
      ...
      if (cacheResp && path.startsWith("/apps/")) {
        ...
        if (
          ...
          cacheResp.headers.get("content-type").match(/^(?:text)\/html/g)
        ) {
          body = await body.text();
          ...
          cacheResp.headers.get("content-type").match(/^(?:text)\/css/g)
        ) {
          return new Response(body, {
            headers: {
              "content-type": getContentType(req.url),
